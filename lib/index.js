...
function makeAudioDriver(options) {
  ...
  var onready$ = _xstream2.default.create({
    start: function start(l) {
      _soundmanager.soundManager.setup({
        onready: function onready() {
          return l.next(_soundmanager.soundManager);
        }
      });
    },
    stop: function stop() {}
  ...
  var audioDriver = function audioDriver(audio$) {
    ...
    var out$ = _xstream2.default.create({
      start: function start(l) {
        _this.exec = commandExecutor(l);
        _this.listener = {
          next: function next() {
            return audio$.addListener(_this.exec);
          },
          complete: function complete() {},
          error: function error() {}
        };
        onready$.addListener(_this.listener);
      },
      stop: function stop() {
        onready$.removeListener(_this.listener);
        audio$.removeListener(_this.exec);
      }
    });
  ...
  audioDriver.streamAdapter = _xstreamAdapter2.default;
  return audioDriver;
}
